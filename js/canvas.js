...
function Fireworks(opts) {
	this.renderInterval = 40; // ms between rendered frames
	this.stepInterval = 10; // ticks between timeline steps
	this.drag = 0.01; // velocity lost per frame
	this.gravity = 0.5; // downward acceleration
	this.wind = -0.2; // horizontal slide applied to everything each frame
	this.sprites = { // images used in animation
	...
	this.fov = 300;
	$.extend(this, opts); // Allow opts to override the vars above
	this.imgs = {}; // loaded sprites
	this.loading = 0;
	this.loadSprites(); // block while loading sprites
	this.particles = []; // all particles ever created
	this.spareParticles = []; // particles ready to be reused
	this.tick = 0; // one per rendered frame
	this.step = 0; // one per timeline event
	this.timer = null;
}

Fireworks.prototype.initCanvas = function(id) {
